...
private fun LiveStatusCard(uiState: MainUiState, onAction: (MainAction) -> Unit) {
    ...
    ElevatedCard(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text(
                "Current altitude", style = MaterialTheme.typography.titleMedium,
...
// ─── Monitoring card (master enable/disable) ─────────────────────────────────

@Composable
private fun MonitoringCard(uiState: MainUiState, onAction: (MainAction) -> Unit) {
    ElevatedCard(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            val (summaryText, summaryIsError) = when (uiState.monitorReadiness) {
                ...
                MonitorReadiness.READY -> when {
                    !uiState.isConfigValid ->
                        "Fix configuration errors before enabling monitoring." to true

                    !uiState.alertsEnabled && uiState.liveStatus.altitudeFeet == null ->
                        "Waiting for altitude reading…" to false

                    uiState.alertsEnabled -> "Monitoring active." to false
                    else -> "Monitoring off." to false
                }
            }
            Text(
                summaryText,
            ...
            Button(
                ...
                Text(
                    if (uiState.alertsEnabled) "Disable monitoring" else "Enable monitoring",
...
private fun BandAlertCard(uiState: MainUiState, onAction: (MainAction) -> Unit) {
    ...
    val isCrossedUnmuted = uiState.alertsEnabled &&
            overallStatus == AlertStatus.CROSSED && !uiState.crossingAlarmMuted
    ...
    ElevatedCard(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            if (uiState.alertsEnabled && overallStatus != null) {
                val (chipBg, chipFg) = when (overallStatus) {
            ...
            // ── Mute — only while band crossing alarm is active ───────────────
            if (isCrossedUnmuted) {
                OutlinedButton(
